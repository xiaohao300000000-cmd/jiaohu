...
export async function handlePupuAddressRequest(
  ...
  if (request.method === "POST" && url.pathname.endsWith("/addresses/select")) {
    ...
    if (!body || typeof body.receiverId !== "string" || !body.receiverId) {
      return json({ error: { code: "invalid_address" } }, 400);
    }
    const selected = await dependencies.controller.select(scope, body.receiverId);
    return json({ selected: true, addressId: selected.receiverId });
  }
