...
import type { createPupuCommerceClient } from "../../ai/pupu-commerce-client";
...
export function PupuCheckoutJourney({
  task: initialTask,
  commerce,
}: Props) {
  ...
  async function load() {
    ...
    setError("");
    try {
      const result = await commerce.previewCheckout({
        taskId: task.taskId,
  ...
  async function create() {
    ...
    try {
      ...
      setError("订单结果未能确认。请勿重复下单，先重新查询当前状态。");
    } finally {
      setBusy(false);
    }
  }
  ...
  if (payment) {
    return (
      <section className="pupu-checkout" aria-label="朴朴待付款订单">
        <div className="pupu-checkout__status">
          <ShieldCheck size={18} aria-hidden="true" />
          <strong>真实待付款订单已创建</strong>
        </div>
        ...
        <small>
          点击只会打开朴朴官方页面；系统不会代你付款，也不会因点击而标记成功。
        </small>
      </section>
    );
  }
      ...
      {!preview ? (
        ...
        <>
          ...
          <dl>
            <div>
              <dt>商品</dt>
              <dd>¥{(preview.productTotalCents / 100).toFixed(2)}</dd>
            </div>
            <div>
              <dt>配送费</dt>
              <dd>¥{(preview.deliveryFeeCents / 100).toFixed(2)}</dd>
            </div>
            <div>
              <dt>优惠</dt>
              <dd>-¥{(preview.discountCents / 100).toFixed(2)}</dd>
            </div>
          </dl>
          <p>这一步尚未创建订单</p>
          <button
