...
function commerce(commitResult: Promise<{ status: string }> = Promise.resolve({ status: "verified" })) {
  const afterPreview = {
    ...
    context: {
      ...task.context,
      cartPreview: { id: "cart-confirmation", version: 3, expiresAt: "2999-01-01T00:00:00Z" },
    },
  };
  const afterCommit = {
  ...
  return {
    previewCart: vi.fn().mockResolvedValue({
      confirmationId: "cart-confirmation", totalCents: 1390, task: afterPreview,
    }),
...
describe("PupuCartConfirmCard", () => {
  it("shows exact consequences and requires a separate task-bound commit click", async () => {
    ...
    render(<PupuCartConfirmCard task={task} commerce={client as never} />);
